...
def sort_call_seq_for_exit(c, call_seq_out):
    ...
    last_position_min = (c.last_position).argmin()
    
    first_half = np.arange(int(c.group_len / 2))
    second_half = np.arange(int(c.group_len / 2), c.group_len)
    
    if last_position_min < int(c.group_len / 2):
...
def get_entry_orders(c, funding_debit, best_spread_idx, bt_args):
    ...
    log(bt_args.logging, "leverage_factors", leverage_factors)
...
# @njit
# def get_expected_profit(spread_idx, c, bt_args):
#     if not bt_args.profitable_only:
#         return 1

#     current_zscore = bt_args.zscore[c.i, spread_idx]
#     current_prices = index_both_legs(c.close[c.i], spread_idx)
#     final_price = sum(current_prices) / 2

#     fee_pct = bt_args.fee_pct[spread_idx]
#     fee_fixed = bt_args.fee_fixed[spread_idx]
#     slippage = bt_args.bid_ask_spread[c.i, spread_idx] / current_prices
#     init_margin = bt_args.init_margin[spread_idx]
#     maint_margin = bt_args.maint_margin[spread_idx]
#     var = bt_args.var[spread_idx]
#     max_vars = np.array([max(np.abs(var[0])), max(np.abs(var[1]))])
...
def update_funding_debit(c, funding_debit, bt_args):
    ...
    for i in range(c.group_len):
        ...
        funding_debit[i] += funding_rate[0] * last_position[0] * prices[0]
        funding_debit[i] += funding_rate[1] * last_position[1] * prices[1]
...
def flex_order_func_nb(
    c, directions, spread_dir, funding_debit, call_seq_out, held_spread_idx, best_spread_idx, action, entry_orders, exit_orders, bt_args
):
    ...
    if action == ENTRY:
        if col not in entry_indices:
            return col, nb.order_nothing_nb()
